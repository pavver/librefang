...
impl ChannelsResource {
    ...
    pub async fn list_channel_registry(&self) -> Result<Value> {
        do_req(&self.client, &self.base_url, reqwest::Method::GET, &"/api/channels/registry".to_string(), None, &[]).await
    }
    ...
    pub async fn configure_sidecar_channel(&self, name: &str, data: Value) -> Result<Value> {
        do_req(&self.client, &self.base_url, reqwest::Method::POST, &format!("/api/channels/sidecar/{}/configure", name), Some(data), &[]).await
    }
}
...
impl ModelsResource {
    ...
    pub async fn list_credential_pools(&self) -> Result<Value> {
        do_req(&self.client, &self.base_url, reqwest::Method::GET, &"/api/credential-pools".to_string(), None, &[]).await
    }
...
impl SessionsResource {
    ...
    pub async fn patch_session_model(&self, id: &str, data: Value) -> Result<Value> {
        do_req(&self.client, &self.base_url, reqwest::Method::PATCH, &format!("/api/sessions/{}/model", id), Some(data), &[]).await
    }
}
